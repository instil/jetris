...
public class GameControllerTest {
    ...
    private final DeterministicGameClock clock = new DeterministicGameClock();

    private final GameModelListener modelListener = mock(GameModelListener.class);

    private final GameController gameController = new GameController(modelListener, clock);
    ...
    public void shouldInformModelListenerOfActiveStateWhenPlayStarts() throws Exception {
        ...
        assertThat(clock.isStarted(), is(true));
        assertThat(clock.isPaused(), is(false));
    }
    ...
    public void shouldPauseSchedulerAndInformModelListenerOfPausedStateWhenGamePaused() throws Exception {
        ...
        assertThat(clock.isStarted(), is(true));
        assertThat(clock.isPaused(), is(true));
    }
    ...
    public void shouldStopSchedulerAndPublishEndedModelWhenGameEnded() throws Exception {
        ...
        assertThat(clock.isStarted(), is(false));
    }
    ...
    private static GameState pausedState() {
        return GameState.PAUSED;
    }

    private static GameState endState() {
        return GameState.ENDED;
    }

    private static Matcher<List<Block>> aTetromino() {
    ...
    private static Matcher<List<Block>> firstTetrominoAtX(final int x) {
        return new BaseMatcher<List<Block>>() {
            @SuppressWarnings("unchecked") @Override
            public boolean matches(Object item) {
    ...
    private static Matcher<List<Block>> firstTetrominoAtY(final int y) {
        return new BaseMatcher<List<Block>>() {
            @SuppressWarnings("unchecked") @Override
            public boolean matches(Object item) {
    ...
    private static Matcher<List<Block>> aTetrominoAtY(final int y) {
        return new BaseMatcher<List<Block>>() {
            @SuppressWarnings("unchecked") @Override
            public boolean matches(Object item) {
                for (Block block : (List<Block>) item) {
